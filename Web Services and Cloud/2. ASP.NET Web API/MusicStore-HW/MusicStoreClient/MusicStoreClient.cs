    ...
    internal class MusicStoreClient
    {
        ...
        private static void PrintSongs(int songId)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                if (songId > 0)
                {
                    SongDto song = response.Content.ReadAsAsync<SongDto>().Result;
        ...
        private static void PrintAlbums(int albumId)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    IEnumerable<Album> albums = response.Content.ReadAsAsync<IEnumerable<Album>>().Result;
        ...
        private static void PrintArtists(int artistId)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    IEnumerable<Artist> artists = response.Content.ReadAsAsync<IEnumerable<Artist>>().Result;
        ...
        private static void AddSong(string title, int? year, string genre, string artistName, string albumTitle)
        {
            ...
            HttpResponseMessage response = Client.PostAsXmlAsync(
                                                                 string.Format(
                                                                               "api/Songs?artistName={0}&albumTitle={1}", 
                                                                               artistName, 
                                                                               albumTitle), 
                                                                 newSong).Result;
            ...
            if (response.IsSuccessStatusCode)
            {
                Uri newSongUri = response.Headers.Location;
        ...
        private static void AddAlbum(string title, int? year, string producer, string artistName)
        {
            ...
            HttpResponseMessage response = Client.PostAsJsonAsync(
                                                                  string.Format(
                                                                                "api/Albums?artistName={0}", 
                                                                                artistName), 
                                                                  newAlbum).Result;

            if (response.IsSuccessStatusCode)
            {
                Uri newAlbumUri = response.Headers.Location;
        ...
        private static void AddArtist(string name, string country, DateTime? dateOfBirth)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                Uri newArtistUri = response.Headers.Location;
