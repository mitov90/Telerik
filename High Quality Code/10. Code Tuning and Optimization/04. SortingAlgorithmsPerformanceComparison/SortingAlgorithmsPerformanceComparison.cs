...
internal class SortingAlgorithmsPerformanceComparison
{
        ...
        where T : IComparable<T>
    {
        ...
        var array1 = (T[])array.Clone();
        var array2 = (T[])array.Clone();
        var array3 = (T[])array.Clone();

        var insertionSortMessage = string.Format("Insertion sort for {0}[]", typeof(T).Name).PadRight(35, '.') + ": ";

        DisplayExecutionTime(() => SortingAlgorithms.InsertionSort(array1), insertionSortMessage);

        var selectionSortMessage = string.Format("Selection sort for {0}[]", typeof(T).Name).PadRight(35, '.') + ": ";

        DisplayExecutionTime(() => SortingAlgorithms.SelectionSort(array2), selectionSortMessage);

        var quicksortMessage = string.Format("Quicksort for {0}[]", typeof(T).Name).PadRight(35, '.') + ": ";

        DisplayExecutionTime(() => SortingAlgorithms.Quicksort(array3, 0, array3.Length - 1), quicksortMessage);
    }

    private static void Main()
    {
        var intsArray = GetRandomIntArray(20000);
        var doublesArray = GetRandomDoubleArray(20000);
        var stringsArray = GetRandomStringArray(20000, 100);
