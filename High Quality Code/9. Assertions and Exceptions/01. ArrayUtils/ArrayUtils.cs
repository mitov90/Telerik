...
public static class ArrayUtils
{
        ...
        where T : IComparable<T>
    {
        ...
        var index = BinarySearch(array, value, 0, array.Length - 1);
        return index;
    }
    ...
    private static int FindMinElementIndex<T>(IList<T> array, int startIndex, int endIndex)
        where T : IComparable<T>
    {
        Debug.Assert(array != null, "Array is null.");
        Debug.Assert(array.Count > 0, "Array is empty");
        Debug.Assert(
            startIndex >= 0 && startIndex < array.Count,
            "startIndex is invalid.",
            "startIndex is not in the range between 0 and {0}.",
            array.Count - 1);
        Debug.Assert(
            endIndex >= 0 && endIndex < array.Count,
            "endIndex is invalid.",
