namespace DirectoryTreeBuilder
{
    ...
    internal class DirectoryTreeBuilder
    {
        ...
        {
            var root = (DirectoryInfo)rootNode.Data;
            ...
            {
                var childNode = rootNode.Nodes.Add(subdirectory);
        ...
        private static void Run()
        {
            ...
            var directoryTree = new Tree<FileSystemInfo>(rootDirectory);

            BuildDirectoryTree(directoryTree.Root);

            foreach (var parentNode in directoryTree.Root.Filter(fi => fi is DirectoryInfo))
            {
                var totalFileSize = parentNode.Accumulate(
                    0L,
                    (acc, fi) =>
                        {
                            var fileInfo = fi as FileInfo;
                            return fileInfo != null ? acc + fileInfo.Length : 0;
                        },
                    fi => fi is FileInfo);
