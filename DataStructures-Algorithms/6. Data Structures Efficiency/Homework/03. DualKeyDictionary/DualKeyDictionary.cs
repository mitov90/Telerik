...
public class DualKeyDictionary<TKey1, TKey2, TValue> : IEnumerable<KeyValuePair<TKey1, TValue>>
{
    ...
    public void Associate(TKey2 subKey, TKey1 primaryKey)
    {
        if (!this.baseDictionary.ContainsKey(primaryKey))
        {
            throw new KeyNotFoundException(
                string.Format("The base dictionary does not contain the key '{0}'", primaryKey));
        }

        TKey2 value;
        if (this.primaryToSubKeyMapping.TryGetValue(primaryKey, out value))
        {
    ...
    {
        var primaryKey = this.subDictionary[subKey];
        this.baseDictionary.Remove(primaryKey);
    ...
    {
        var values = new TValue[this.baseDictionary.Values.Count];
    ...
    {
        var primaryKeys = new TKey1[this.baseDictionary.Keys.Count];
    ...
    {
        var subKeys = new TKey2[this.subDictionary.Keys.Count];
