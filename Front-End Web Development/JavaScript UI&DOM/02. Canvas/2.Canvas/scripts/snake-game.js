var snakeGame = function(canvas) {
    ...
    function setupGame() {
        ...
        for (var i = 0, length = apples.length; i < length; i++) {
            placeApples(i);
        }
    };
    ...
    function checkForCollisions() {
        ...
        for (var i = 0,length=apples.length; i < length; i++) {
            if (head.row === apples[i].row && head.col === apples[i].col) {
                segmentsToGrow += GROWTH;
                score += 1;
                setScore();
                placeApples(i);
    ...
    function drawApple() {
        for (var i = 0, length = apples.length; i < length; i++) {
            ...
            drawInCell(apples[i].row, apples[i].col, function() {
